...
addons_path = openerp.tools.config['addons_path'] + '/auto_backup/DBbackups'
    ...
    def schedule_backup(self, cr, user, context={}):
        ...
        for rec in confs:
            ...
            if rec.name in db_list:
                ...
                bkp=''
                try:
                    bkp = execute(conn, 'dump', openerp.tools.config['admin_passwd'], rec.name)
                except:
                    logger.info("Couldn't backup database %s. Bad database administrator password for server running at http://%s:%s" %(rec.name, rec.host, rec.port))
                    continue
                bkp = base64.decodestring(bkp)
                fp.write(bkp)
                fp.close()
            else:
                logger.info("database %s doesn't exist on http://%s:%s" %(rec.name, rec.host, rec.port))
